      ...
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none select-none">
        ...
        <div className="absolute top-4 right-4 md:top-6 md:right-6 pointer-events-auto z-50 flex flex-col gap-3">
          <button
            ...
            className={`group relative p-3 border transition-all duration-300 ${isIdle
        ...
        <div className="absolute bottom-8 left-0 pointer-events-auto z-50 group">
          <div className="transform -translate-x-[calc(100%-3.25rem)] group-hover:translate-x-0 transition-transform duration-300 ease-in-out">
            <button
              ...
              className={`relative p-3 border-y border-r rounded-r-xl transition-all duration-300 flex items-center gap-3 ${spsMode
            ...
            <div className="flex flex-col gap-1 bg-black/40 backdrop-blur-md border-l border-white/10 p-1">
              {Object.entries(ENVIRONMENTS).map(([key, env]) => (
                <button
                  ...
                  className={`p-2 md:p-3 transition-all relative group flex items-center justify-end gap-3 ${lighting === key
                      ? 'bg-white text-black'
            ...
            <div className="flex flex-col gap-2 items-end">
              {Object.entries(SCENES).map(([key, scene]) => (
                <button
                  ...
                  className={`w-8 h-8 md:w-10 md:h-10 transition-all relative group border border-white/10 overflow-hidden ${sceneType === key ? 'border-white scale-110 shadow-[0_0_15px_rgba(255,255,255,0.3)]' : 'opacity-50 hover:opacity-100 hover:border-white/40'
                    }`}
                  title={scene.name}
        ...
        >
          <div className="bg-black/80 backdrop-blur-xl border-t md:border border-white/20 p-1 shadow-2xl relative">
            ...
            <div className={`${mobileTab === 'car' ? 'flex' : 'hidden'} md:flex p-4 md:p-6 border border-white/5 flex-col gap-4 md:gap-6`}>
              {/* Finishes */}
              <div className="flex justify-center gap-px bg-white/5 p-1 overflow-x-auto">
                {FINISHES.map((f) => (
                  <button
                    ...
                    className={`px-3 py-2 md:px-6 text-[10px] md:text-xs font-mono uppercase tracking-wider transition-all flex-1 whitespace-nowrap ${finish === f.id
                    ...
                    <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
                      {LUNACY_MATERIALS.map((mat) => (
                        <button
                          ...
                          className={`group relative aspect-square border transition-all overflow-hidden ${lunacyMaterial === mat.id && !customTextureUrl
                  ...
                  <>
                    ...
                    <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide justify-start items-center px-1">
                      ...
                      <button
                        ...
                        className={`relative group w-14 h-14 border transition-all flex-shrink-0 overflow-hidden ${!currentColors.some(c => c.value === color) ? 'border-white' : 'border-white/10 hover:border-white/40'
                          }`}>
                      ...
                      {currentColors.map((c) => (
                        <button
                          ...
                          className={`group relative w-12 h-12 transition-all flex-shrink-0 ${color === c.value ? 'scale-110 z-10' : 'hover:scale-105 opacity-80 hover:opacity-100'
                            }`}
                          title={c.name}
                        >
                          <div
            ...
            <div className={`${mobileTab === 'env' ? 'flex' : 'hidden'} md:hidden p-4 flex-col gap-6`}>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                  {Object.entries(ENVIRONMENTS).map(([key, env]) => (
                    <button
                      key={key}
                      onClick={() => setLighting(key as any)}
                      className={`p-3 border transition-all flex-shrink-0 flex flex-col items-center gap-2 min-w-[80px] ${lighting === key
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                  {Object.entries(SCENES).map(([key, scene]) => (
                    <button
                      key={key}
                      onClick={() => setSceneType(key as any)}
                      className={`p-1 border transition-all flex-shrink-0 relative group overflow-hidden w-24 h-12 ${sceneType === key ? 'border-white shadow-[0_0_10px_rgba(255,255,255,0.3)]' : 'border-white/10 opacity-60'
                        }`}
                      style={{ backgroundColor: scene.color }}
                    >
